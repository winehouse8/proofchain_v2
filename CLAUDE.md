...
## src/ 수정 전 필수 절차 (ASIL B 자동 강제)

`src/` 파일을 수정하기 **전에** 반드시 아래 절차를 자율적으로 수행하라. 사람이 요청하지 않아도 Claude가 스스로 해야 한다.

1. **TC 파일 조회**: 수정 대상 파일이 속한 영역(CV/CT/CG)의 TC 파일을 읽는다.
   - CV → `.omc/test-cases/TC-CC-CV.json`
   - CT → `.omc/test-cases/TC-CC-CT.json`
   - CG → `.omc/test-cases/TC-CC-CG.json`

2. **관련 TC/REQ 식별**: 수정 내용과 관련된 TC ID와 REQ ID를 찾는다.
   - 없으면 사람에게 "이 수정이 커버하는 TC/REQ를 찾지 못했습니다" 보고 후 확인 요청

3. **어노테이션 추가**: 수정하는 소스 파일 상단 또는 해당 함수 위에 추가:
   ```
   // @tc: TC-CC-XX-001, TC-CC-XX-002
   // @req: REQ-CC-XX-01
   ```

4. **hitl-state.json log 기록**: auto-backward 발생 시 log 항목에 `tc_ids`, `req_ids` 필드 포함:
   ```json
   {
     "timestamp": "...",
     "area": "CV",
     ...
     "actor": "claude",
     "tc_ids": ["TC-CC-CV-001"],
     "req_ids": ["REQ-CC-CV-01"],
     "note": "수정 이유"
   }
   ```

이 절차를 건너뛰면 ASIL B 훅이 `exit 2`로 차단한다.
